...
main(int argc, char *argv[])
{
  ...
  TAutomat *automata = NULL;
   ...
   /*=========================================================Para Datos Erroneos*/ 
    ...
    int modo_f, prob_error, err_ack,prob_perdida,perdida;
    char user_e,intro_b;
...
prob_perdida = atoi(argv[4]);
retardo_pr =  atoi(argv[5]);

retardo_pr= retardo_pr *1000;
				...
				switch (modo_f)
					...
					//Modo manual (=1)
						  ...
						  //leemos decisión del usuario por entrada estándar
						   ...
						   intro_b =getchar(); 
                                                  perdida= atoi(&user_e);
					       printf(" Se ha introducido en perdida %i  \n",perdida);
					            err_ack=1;  
                                                   if( perdida==0){
                                                             printf("Introduzca : \n"); 
                                                             printf("   0 = No errores en la trama \n"); 
                                                             printf("   1 = Hay errores en la trama \n"); 
                                                             user_e=getchar();
				           		     intro_b =getchar(); 
                                                             err_ack= atoi(&user_e);}
          				          sigprocmask(SIG_SETMASK, &oldmask, NULL);
					       printf(" Se ha introducido en error %i  \n",err_ack);
					       break;
				   ...
				   printf("*****************antes de 1 compro  \n");
                                    if ((err_ack == 1)&&(strlen(trama.data) !=0)){
                                            trama.control='E';        
                                                  }
                     printf("*****************antes de segunda compro \n");
                       if (perdida==0)
                       {     
                             usleep(retardo_pr);                         
			  if (trama.control=='T'){
				    ...
				    if (strlen(paquete.data) ==0 ){
				               ...
				               printf(" \n Para \n");
                                         ...
                                         automat_transit(argu->active_en,automata,Tra_Vacia, argu);  
                                          ...
                                          //Si la secuencia es la esperada
                                          if (argu->sec_actual == trama.address){
				             ...
				             printf("Escribe en el fichero");
                                            argu->sec_actual= argu->sec_siguiente;
                                               switch(argu->sec_actual)
                                                   {
                                                    case 'A' : { printf("Estoy en A espero B");
                                                                  argu->sec_siguiente='B';   
                                                                 break;}

                                                    case 'B' : { printf("Estoy en A espero B");
                                                                  argu->sec_siguiente='A';   
                                                                 break;}
               				        default : { printf("es un estado no designado = %i \n"); break; }
                                                    } 
                                                argu->sec_ACK= argu->sec_actual;
                                              }
                                             else{
                                                   argu->sec_ACK= trama.address;
                                                   printf("Se envia ACK de trama ya recibida");    
                                                  }
                                               /* entidad , automata, evento, argumentos de evento*/ 
			...
			Argumentos * argu )
{
  ...
  /*Envio ACK*/
               ...
               argu->trama_e->address = argu->sec_ACK;
			...
			Argumentos * argu )
{
  ...
  /*Envio NACK*/
               ...
               argu->trama_e->address = argu->sec_NAK; 
