...
/*==============================================================
memset(paquete.data,'\0',sizeof(paquete.data));
/*devuelve el nde caract leidos*de arch */
/*
n_carac= from_network_layer (&paquete,arch);

//para la trama con datos
memset(trama.data,'\0',sizeof(trama.data));
 trama.control='T';//0x09;

/***********************************/
/*
camb_intro( paquete.data);

formato_trama(&trama,paquete,n_carac);

 
  //enviamos trama de datos 
 to_physical_layer(&trama,fd);
    printf("adress , control %i %i: \n",trama.address,trama.control);
