...
/* Hace el descriptor de archivo asincrono*/
   ...
   tcgetattr(fd,&oldtio); /* salvamos conf. actual del puerto, obtiene los par�etros asociados con el objeto 
...
*/
   ...
   newtio.c_cc[VMIN]=1;
   newtio.c_cc[VTIME]=0;
   tcflush(fd, TCIFLUSH); /* descarta datos recibidos, al objeto referido por fd, pero no le�os,(por TCIFLUSH)*/
   tcsetattr(fd,TCSANOW,&newtio); /* establece los par�etros asociados con la terminal(puerto) desde la estructura 
 ...
 */
                if (wait_flag==FALSE) {
		                  tam_trama = from_physical_layer(&trama, fd);  
				  printf("control : %c \n", trama.control);
			  ...
			  if (trama.control=='T'){
				  ...
				  printf(" trama de datos \n");
				   ...
				   printf("paquete %s",paquete.data);                                 
				    if (strlen(paquete.data) ==0 ){ STOP=TRUE; /* para el bucle si solo entra un CR */
				                  printf(" \n Para \n");
                                         /* entidad , automata, evento, argumentos de evento */
				      ...
				      {
				      printf("Recibido trama mal=== paquete de n_carac \n");
				      wait_flag = TRUE;   /* espera una nueva entrada */
                                      /* entidad , automata, evento, argumentos de evento */
