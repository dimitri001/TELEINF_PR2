...
void semaphore_timeout_handler( int inSignal, Argumentos * argu, TAutomat *automata);
int semaphore_timeout_set( int inSecs, Argumentos * argu, TAutomat *automata);
int semaphore_timeout_cancel( );
...
TAutomat * s1_automat_define( )
{
  ...
  automat_declare_transition( autom,
		STATE_WAIT_ACK, STATE_WAIT_ACK,
		NACK_ACT, reenviar_trama );

		
  automat_declare_transition( autom,
		STATE_WAIT_ACK, STATE_WAIT_ACK,
		ACK_N_SIG,ack_repetido );

  automat_declare_transition( autom,
		STATE_WAIT_ACK, STATE_WAIT_ACK,
		NACK_N_ACT, nack_n_act );
      ...
      printf(" ===> Enviamos trama. \n");
         ...
         //para la trama con datos
            memset(argu->trama_e->data,'\0',sizeof(argu->trama_e->data));
             argu->trama_e->control='T';//0x09;
             argu->trama_e->address = argu->sec_actual;                 
         /***********************************/
 ...
 * ack_repetido
 * ------------------------------------------------------------ */
int ack_repetido (	TEntity * inEntity,
			TState inFromState,
			TEvent inEvent,
			Argumentos * argu )
{
  int rc = AUTOMAT_OK;
  printf(" ===> Se ha recibido un ACK Repetido. \n");
   print_estado(argu->active_en);
 ...
 *  nack_n_act
 * ------------------------------------------------------------ */
int nack_n_act (	TEntity * inEntity,
			TState inFromState,
			TEvent inEvent,
			Argumentos * argu )
{
  int rc = AUTOMAT_OK;
  printf(" ===> Se ha recibido un NACK quer no corresponde con la secuencia actual. \n");
   print_estado(argu->active_en);
...
void semaphore_timeout_handler( int inSignal, Argumentos * argu, TAutomat *automata)
{
  /* Just for showing argument passing */

  semaphore_timeout_cancel( );

 automat_transit(argu->active_en, automata, TIME_O, argu);
 argu->tr_perdidas ++;
 printf("\n Se ha activado la alarma del temporizador y se reenvia la trama\n" );
 //semaphore_timeout_set(argu->tempo,argu,automata);
  /*automat_transit(	active_sem, semaphore_automat,
			SEM_EV_TIMEOUT, &semaphore_args );*/
}
...
int semaphore_timeout_set( int inSecs, Argumentos * argu, TAutomat *automata)
{
  signal( SIGALRM, semaphore_timeout_handler );
  alarm( inSecs );
